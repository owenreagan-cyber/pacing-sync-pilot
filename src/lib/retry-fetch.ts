/**
 * Shared retry-fetch primitive (browser side).
 *
 * Mirrors `supabase/functions/_shared/retry-fetch.ts` so frontend and edge
 * functions retry Canvas/API calls with the same contract. Keep both files
 * in sync — change the defaults here and there together.
 */
...
/**
 * Fetch with exponential backoff + jitter on retryable status codes.
 * Returns the last response (successful or final failure) — never throws
 * on HTTP status. Network errors are re-thrown after retries are exhausted.
 */
